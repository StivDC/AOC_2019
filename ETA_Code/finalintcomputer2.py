### Update: change list to store program -> defaultdict
###         add run_until_input method hopefully
...
class IntComputer():
    ...
    def __init__(self, program, start_stack = []):
        # Maybe should be lambda: 0 not int
        self.program = defaultdict(int, {i:val for i, val in enumerate(program)})
    ...
    def iteration(self, debug, breakoninput = False):
        ...
        if op in [1, 2]:
            ...
            # Output
            self.outputstack.append(convert(get(self.pc+1), modes[0]))
            self.pc += 2
    ...
    def run_until_output(self, debug = False):
        ...
        while not self.finished:
            ...
            if self.iteration(debug) == "output": # Break on ouput
                #print(self.pc)
                return
    ...
    def run_until_input(self, debug = False):
        """Run until input is required"""
        while not self.finished:
            if self.iteration(debug) == "input": # Break on input
                return
...
for prog, val, result in zip(tests, vals, results):
    ...
    comp.run()
    assert(comp.outputstack[0] == result[0])
    comp = IntComputer(prog[:], [val[1]])
    comp.run()
    assert(comp.outputstack[0] == result[1])
